...
**选择语言:**
- [英文](README.md) • [中文(简体)](README.zh-CN.md)
...
PSI 采用**基于时间窗口的多指数加权融合方法**，全面考虑情绪变化和负面情绪持续性的时间特性。计算过程如下：
...
| `w₁` | 负面情绪的权重比例 | 0–1 | 0.4 |
| `w₂` | 情绪波动熵的权重 | 0–1 | 0.3 |
| `w₃` | 正常化负面情绪的持续性 | 0–1 | 0.3 |
| `P₊` | 积极情绪比率（时间窗口内积极情绪占比） | 0–1 | — |
| `E`  | 情绪波动熵（归一化香农熵，值高→不稳定） | 0–1 | — |
| `D₋` | 负面情绪持续性归一化（持续时长 / 最大允许时长） | 0–1 | — |
| `clip(..., 0, 1)` | 约束中间结果 ∈ [0, 1] | — | — |
...
| PSI 范围 | 等级 | 描述 |
|----------|------|------|
| 0–30 | **良好** | 心理状态稳定，积极情绪占主导，波动低 |
| 31–60 | **中等** | 中度情绪波动或偶尔负面情绪 |
| 61–100 | **差** | 持续负面情绪、高度情绪不稳定或持续负面状态 |

---

### 配置灵活

- 可在 `config/config.yaml` 的 `psi_time_based` 节自定义：  
  - 权重 `w₁, w₂, w₃`  
  - 情绪类别  
  - 时间窗口参数  
  - 等级阈值
- 系统自动验证并修正无效配置（负权重、不合理阈值等），确保计算鲁棒性

## 示例结果
（可视化Web前端本项目不提供，可自行设计）
